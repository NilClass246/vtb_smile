 ...
 *   作用于所有贴图。
 * 2.纯色滤镜、着色滤镜、模糊滤镜……等 都相互独立，且效果可以相互叠加。
 ...
 * 6.注意，在1.61盗版rmmv中，滤镜核心可能会出现图像拉扯的小问题。
...
//		性能消耗分布：	模糊滤镜 > 噪点滤镜 > 着色滤镜 > 纯色滤镜 == 填充滤镜
...
Sprite.prototype.drill_COF_initializeSprite = function(){
	this._drill_COF_s = {};
	this._drill_COF_s.pureBoard = null;		//>纯色滤镜
	this._drill_COF_s.colorBoard = null;	//>着色滤镜
	this._drill_COF_s.fillBoard = null;		//>填充滤镜
	this._drill_COF_s.rainbowBoard = null;	//>彩虹滤镜
	this._drill_COF_s.blurFilter = null;	//>模糊滤镜
	this._drill_COF_s.noiseFilter = null;	//>噪点滤镜
...
// * 滤镜 - 框架一致（滤镜板）
//==============================
var _drill_COF_setFrame = Sprite.prototype.setFrame;
Sprite.prototype.setFrame = function(x, y, width, height) {
	_drill_COF_setFrame.call(this, x, y, width, height);
	this.drill_COF_frameChanged();
}
Sprite.prototype.drill_COF_frameChanged = function() {
	if( !this._drill_COF_inited ){ return; }
...
Sprite.prototype.drill_COF_updateSprite = function() {
	...
	this.drill_COF_initColorBoard();
	this.drill_COF_initPureBoard();		//纯色在着色滤镜前面
	this.drill_COF_initFillBoard();
	this.drill_COF_initBlurFilter();
	this.drill_COF_initNoiseFilter();
		
	if( this.bitmap && this.bitmap.isReady() ){
		this.drill_COF_updatePureBoard();
		this.drill_COF_updateColorBoard();
		this.drill_COF_updateFillBoard();
		this.drill_COF_updateBlurFilter();
		this.drill_COF_updateNoiseFilter();
	}
}
...
// ** 纯色 - 初始化
//==============================
Sprite.prototype.drill_COF_initPureBoard = function() {
	var pB = this._drill_COF.pureBoard;
	if( pB.opened == false ){ return }
	if( pB.inited != false ){ return }
	...
	temp_sprite._pose = -1;
	temp_sprite._drill_url = this.bitmap ? this.bitmap.url : "";
	this.addChild(temp_sprite); 
	this._drill_COF_s.pureBoard = temp_sprite;
...
// ** 纯色 - 帧刷新
//==============================
Sprite.prototype.drill_COF_updatePureBoard = function() {
	var pB          = this._drill_COF.pureBoard;
	var temp_sprite = this._drill_COF_s.pureBoard;
	if( pB.opened == false ){ return }
	if( !temp_sprite ){ return; };
...
Sprite.prototype.drill_COF_setPureWave = function(type_,range_,period_) {
	...
	pB.mode = "wave";
	pB.opened = true;
	if( pB.o_range == 0 && this._drill_COF_s.pureBoard ){
		this._drill_COF_s.pureBoard.opacity = 0;
	}
...
// ** 着色 - 初始化
//==============================
Sprite.prototype.drill_COF_initColorBoard = function() {
	var cB = this._drill_COF.colorBoard;
	if( cB.opened == false ){ return }
	if( cB.inited != false ){ return }
	...
	temp_sprite._drill_url = this.bitmap ? this.bitmap.url : "";
	this.addChild(temp_sprite); 
	this._drill_COF_s.colorBoard = temp_sprite;
...
// ** 着色 - 帧刷新 
//==============================
Sprite.prototype.drill_COF_updateColorBoard = function() {
	var cB          = this._drill_COF.colorBoard;
	var temp_sprite = this._drill_COF_s.colorBoard;
	if( cB.opened == false ){ return }
	if( !temp_sprite){ return; };
...
Sprite.prototype.drill_COF_setColorWave = function(type_,range_,period_) {
	...
	cB.mode = "wave";
	cB.opened = true;
	if( cB.o_range == 0 && this._drill_COF_s.colorBoard ){
		this._drill_COF_s.colorBoard.opacity = 0;
	}
...
// ** 填充 - 初始化
//==============================
Sprite.prototype.drill_COF_initFillBoard = function() {
	var fB = this._drill_COF.fillBoard;
	if( fB.opened == false ){ return }
	if( fB.inited != false ){ return }
	...
	temp_sprite._drill_url = this.bitmap ? this.bitmap.url : "";
	this.addChild(temp_sprite); 
	this._drill_COF_s.fillBoard = temp_sprite;
...
// ** 填充 - 帧刷新
//==============================
Sprite.prototype.drill_COF_updateFillBoard = function() {
	var fB          = this._drill_COF.fillBoard;
	var temp_sprite = this._drill_COF_s.fillBoard;
	if( fB.opened == false ){ return }
	if( !temp_sprite ){ return; };
...
Sprite.prototype.drill_COF_setFillWave = function(type_,range_,period_) {
	...
	fB.mode = "wave";
	fB.opened = true;
	if( fB.o_range == 0 && this._drill_COF_s.fillBoard ){
		this._drill_COF_s.fillBoard.opacity = 0;
	}
...
// ** 彩虹 - 初始化
//==============================
Sprite.prototype.drill_COF_initRainbowBoard = function() {
	var rB = this._drill_COF.rainbowBoard;
	if( rB.opened == false ){ return }
	if( rB.inited != false ){ return }
	...
	temp_sprite._drill_url = this.bitmap ? this.bitmap.url : "";
	this.addChild(temp_sprite); 
	this._drill_COF_s.rainbowBoard = temp_sprite;
...
// ** 彩虹 - 帧刷新
//==============================
Sprite.prototype.drill_COF_updateRainbowBoard = function() {
	var rB          = this._drill_COF.rainbowBoard;
	var temp_sprite = this._drill_COF_s.rainbowBoard;
	if( rB.opened == false ){ return }
	if( !temp_sprite ){ return; };
...
Sprite.prototype.drill_COF_setRainbowWave = function(type_,range_,period_) {
	...
	rB.mode = "wave";
	rB.opened = true;
	if( rB.o_range == 0 && this._drill_COF_s.rainbowBoard ){
		this._drill_COF_s.rainbowBoard.opacity = 0;
	}
...
// ** 模糊 - 初始化
//==============================
Sprite.prototype.drill_COF_initBlurFilter = function() {
	var bF = this._drill_COF.blurFilter;
	if( bF.opened == false ){ return }
	if( bF.inited != false ){ return }
	
	// >建立滤镜
	this._drill_COF_s.blurFilter = new PIXI.filters.BlurFilter();
	if( this.filters == null){		//滤镜并不像数组一样可以直接push，需要个中介
		var f_intermediary = [];
	}else{
		var f_intermediary = this.filters;
	}
	f_intermediary.push(this._drill_COF_s.blurFilter);
	...
	this._drill_COF_s.blurFilter.blur = bF.o_cur / 255 * 8;
	this._drill_COF_s.blurFilter.quality = 1 + bF.o_cur / 255;
}
//==============================
// ** 模糊 - 帧刷新
//==============================
Sprite.prototype.drill_COF_updateBlurFilter = function() {
	var bF          = this._drill_COF.blurFilter;
	var temp_filter = this._drill_COF_s.blurFilter;
	if( !temp_filter ){ return; }
...
Sprite.prototype.drill_COF_setBlurWave = function( range_,period_) {
	...
	bF.mode = "wave";
	bF.opened = true;
	if( bF.o_range == 0 && this._drill_COF_s.blurFilter ){
		this._drill_COF_s.blurFilter.blur = 0;
	}
...
// ** 噪点 - 初始化
//==============================
Sprite.prototype.drill_COF_initNoiseFilter = function() {
	var nF = this._drill_COF.noiseFilter;
	if( nF.opened == false ){ return }
	if( nF.inited != false ){ return }
	
	// >建立滤镜
	this._drill_COF_s.noiseFilter = new PIXI.filters.NoiseFilter();
	if( this.filters == null){		//滤镜并不像数组一样可以直接push，需要个中介
		var f_intermediary = [];
	}else{
		var f_intermediary = this.filters;
	}
	f_intermediary.push(this._drill_COF_s.noiseFilter);
	this.filters = f_intermediary;
	...
	this._drill_COF_s.noiseFilter.noise = nF.o_cur / 255 * 4;
	this._drill_COF_s.noiseFilter.seed = Math.random();
}
//==============================
// ** 噪点 - 帧刷新
//==============================
Sprite.prototype.drill_COF_updateNoiseFilter = function() {
	var nF          = this._drill_COF.noiseFilter;
	var temp_filter = this._drill_COF_s.noiseFilter;
	if( !this._drill_COF_s.noiseFilter ){ return; }
...
Sprite.prototype.drill_COF_setNoiseWave = function( range_,period_) {
	...
	nF.mode = "wave";
	nF.opened = true;
	if( nF.o_range == 0 && this._drill_COF_s.noiseFilter ){
		this._drill_COF_s.noiseFilter.noise = 0;
	}
