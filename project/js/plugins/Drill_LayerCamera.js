 ...
 * 如果想了解镜头更多的内容，可以去看看"关于地图活动镜头.docx"。
 * ★★最好放在 多层地图背景 插件后面★★
 * 
 * -----------------------------------------------------------------------------
 * ----插件扩展
 * 该插件由于控制了镜头的缩放大小，对下列插件有一定影响：
 * 被扩展：
 ...
 *   (4.镜头稍微旋转12度，有很多特殊代入效果，比如摇晃的船只、塌陷的斜坡、
 ...
 *   (4.镜头翻转时，你可以看到黑色图块边缘，以及离镜头较远的事件。属正常现象。
 * 镜头锁定/看向指定位置：
 ...
 *   (2.镜头墙是一种阻止镜头单向向上/下/左/右移动的墙，如果玩家越过了墙壁线，
 *      则镜头将不再受阻止。可以用镜头墙设置洞穴中隐藏的房间。
 *   (3.由于镜头墙是绑定在事件上的，如果该事件在不断移动，还可以做机关逼近的
 *      镜头地图。
 *   (4.事件注释设置不跨事件页，切换无相关注释的事件页后，墙会被关闭。
 * 视野触发：
 ...
 * 2.注释只能设置一个独立开关，如果先出现触发A开关的注释，然后出
 *   现触发B的注释，那么将以B开关为准。
 * 
 * -----------------------------------------------------------------------------
 * ----插件性能
...
Game_Player.prototype.drill_LCa_updateSpringScroll = function(lastScrolledX, lastScrolledY) {
    ...
    if (y2 > this.centerY()){
		var distance = Math.abs(y2 - this.centerY());
		var speed = Math.min(distance/speedRatio_y,$gameSystem._drill_LCa_speedMax);	//速度上限
		var pixel_d = 1 / $gameMap.tileHeight();
		if( distance < pixel_d ){ speed = distance; }					//最小收敛间距（防止事件贴图出现像素偏移）
		else if( speed < pixel_d * 0.25 ){ speed = pixel_d * 0.25; }	//最小速度（1/4像素）
        $gameMap.scrollDown(speed);
    }
    if (x2 < this.centerX()) {
		var distance = Math.abs(x2 - this.centerX());
		var speed = Math.min(distance/speedRatio_x,$gameSystem._drill_LCa_speedMax);
		var pixel_d = 1 / $gameMap.tileWidth();
		if( distance < pixel_d ){ speed = distance; }				
		else if( speed < pixel_d * 0.25 ){ speed = pixel_d * 0.25; }	
        $gameMap.scrollLeft(speed);
    }
    if (x2 > this.centerX()) {
		var distance = Math.abs(x2 - this.centerX());
		var speed = Math.min(distance/speedRatio_x,$gameSystem._drill_LCa_speedMax);
        ...
        $gameMap.scrollRight(speed);
    }
    if (y2 < this.centerY()) {
		var distance = Math.abs(y2 - this.centerY());
		var speed = Math.min(distance/speedRatio_y,$gameSystem._drill_LCa_speedMax);
		var pixel_d = 1 / $gameMap.tileHeight();
		if( distance < pixel_d ){ speed = distance; }				
		else if( speed < pixel_d * 0.25 ){ speed = pixel_d * 0.25; }	
        $gameMap.scrollUp(speed);
    }
}
//==============================
// * 镜头范围
